...
(function initCrtTerminal(){
  ...
  function openShipsWindow() {
    return openWindow('ships', 'SHIP HANGAR', {
      ...
      onOpen: (win) => {
        ...
        showDetail(G.selectedCarType && CAR_TYPES[G.selectedCarType] ? G.selectedCarType : keys[0]);

        // One shared animation loop spins every ship; hovered ones lift + speed up.
        let last = performance.now();
        function frame(now) {
          if (!openWindows['ships']) return; // window closed → stop the loop
  ...
  const CMDS = {
    help: { desc: 'Displays this list', instant: true, run: () => new Promise(resolve => {
      ...
      setTimeout(() => {
        ...
        printLines(relabelHelp([
          ...
          'Ships - Lists the available ships',
    ...
    ships: { desc: 'open the ship hangar (spinning showcase)', win: true, run: () => openShipsWindow() },
