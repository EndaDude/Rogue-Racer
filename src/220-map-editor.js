...
const ME = {
  ...
  obstacleRot: 0,
  obstacleScale: 1,
  autoLayer: true,    // auto-detect obstacle layer from nearest node/spline (incl. splits)
  autoOrient: true,   // auto-orient obstacle to nearest spline (incl. splits)
  selObs: -1,         // index of the currently selected obstacle (for slider editing)
};
...
// the nearest track node — main loop AND any V2 split-spline (branch) nodes — so
// bridge decks and forks get deck obstacles without touching the layer box.
function meAutoLayerAt(wx, wy) {
  ...
  (ME.wpts || []).forEach(node => {
    consider(node);
    if (Array.isArray(node.branches)) {
      node.branches.forEach(path => (Array.isArray(path) ? path : []).forEach(consider));
    }
  });
  return best;
}

// Sample a fork spline (owner node `a` → branch node list `br` → next main node `b`)
// into a point list, matching how the fork road ribbon is drawn.
function meSampleForkPts(a, br, b) {
  ...
  for (let k = 0; k < m - 1; k++) {
    const c0 = ctrl[Math.max(0, k - 1)], c1 = ctrl[k], c2 = ctrl[k + 1], c3 = ctrl[Math.min(m - 1, k + 2)];
    const steps = (k === m - 2) ? 16 : 15;
    for (let s = 0; s <= steps; s++) { const t = s / 16; samp.push(catmullRom(c0, c1, c2, c3, t)); }
  }
  return samp;
}

// Nearest-segment tangent (radians) of the track at a world point — used to
// auto-orient obstacles to the road direction on placement. Considers the main
// loop AND every V2 split spline, so forks orient their obstacles correctly too.
function meAutoTangentAt(wx, wy) {
  let bestList = null, bi = 0, bd = Infinity;
  const scan = (pts) => {
    if (!pts || pts.length < 2) return;
    for (let i = 0; i < pts.length; i++) {
      const d2 = (pts[i].x - wx) ** 2 + (pts[i].y - wy) ** 2;
      if (d2 < bd) { bd = d2; bi = i; bestList = pts; }
    }
  };
  scan(ME.spline);
  if (ME.trackModel === 'v2' && ME.wpts.length >= 2) {
    ...
    ME.wpts.forEach((a, i) => {
      ...
      brs.forEach(br => { if (Array.isArray(br) && br.length) scan(meSampleForkPts(a, br, b)); });
    });
  }
  if (!bestList) return 0;
  const a = bestList[(bi - 1 + bestList.length) % bestList.length];
  const b = bestList[(bi + 1) % bestList.length];
  // Both the main loop (meRebuildSpline, wpt order) and fork samples
  // (meSampleForkPts, owner→branch→next) are ordered opposite the racing
  // direction, so flip 180° for both to report a racing-forward tangent.
...
// Push ME.obstacle* (rot / scale / layer) into the toolbar's rot/scale/layer inputs.
function meSyncObstacleControls() {
  const rotR = document.getElementById('me-obstacle-rot-range');
  const rotN = document.getElementById('me-obstacle-rot');
  const scR = document.getElementById('me-obstacle-scale-range');
  const scN = document.getElementById('me-obstacle-scale');
  const lyr = document.getElementById('me-obstacle-layer');
  if (rotR) rotR.value = String(Math.round(ME.obstacleRot || 0));
  if (rotN) rotN.value = String(Math.round(ME.obstacleRot || 0));
  if (scR) scR.value = String(Math.round((ME.obstacleScale || 1) * 100));
  if (scN) scN.value = String(Math.round((ME.obstacleScale || 1) * 100));
  if (lyr) lyr.value = String(Math.round(ME.obstacleLayer || 0));
}

// Select an already-placed obstacle so the sliders edit it in place.
function meSelectObstacle(idx) {
  ME.selObs = idx;
  const o = ME.obstacles[idx];
  if (o) {
    ME.obstacleRot = Math.round(o.rot || 0);
    ME.obstacleScale = Math.max(0.4, Math.min(2.2, o.scale || 1));
    ME.obstacleLayer = Math.round(o.layer || 0);
    meSyncObstacleControls();
  }
  meDraw();
}

// Apply the current rot / scale / layer sliders to the selected obstacle (if any).
function meEditSelObs() {
  if (ME.selObs < 0) return;
  const o = ME.obstacles[ME.selObs];
  if (!o) { ME.selObs = -1; return; }
  o.rot = Math.round(ME.obstacleRot || 0);
  o.scale = Math.max(0.4, Math.min(2.2, ME.obstacleScale || 1));
  o.layer = Math.round(ME.obstacleLayer || 0);
  meDraw();
}
...
function meDraw() {
  ...
  ME.obstacles.forEach((o, i) => {
    ...
    ctx.restore();
    // Highlight the selected obstacle so slider edits are visibly targeted.
    if (i === ME.selObs) {
      ctx.save();
...
function meInitEditor(){
  ...
  const obstacleAutoLayerBtn = document.getElementById('me-obstacle-auto-layer');
  const obstacleAutoOrientBtn = document.getElementById('me-obstacle-auto-orient');

  function refreshModeButtons() {
    // Leaving obstacle mode drops the current obstacle selection (and its highlight).
    if (ME.mode !== 'obstacle' && ME.selObs !== -1) ME.selObs = -1;
    ...
    if (hint) hint.textContent = {
      ...
      obstacle: 'Click empty track to place — click a placed object to select it, then use the Rotate/Scale/Layer sliders. Right-click erases.',
  ...
  obstacleScaleRange.oninput=()=>{ obstacleScaleNum.value = obstacleScaleRange.value; ME.obstacleScale = Math.max(0.4, Math.min(2.2, (parseFloat(obstacleScaleRange.value) || 100) / 100)); meEditSelObs(); };
  obstacleScaleNum.oninput=()=>{ obstacleScaleRange.value = obstacleScaleNum.value; ME.obstacleScale = Math.max(0.4, Math.min(2.2, (parseFloat(obstacleScaleNum.value) || 100) / 100)); meEditSelObs(); };
  if (obstacleAutoLayerBtn) obstacleAutoLayerBtn.onclick = () => { ME.autoLayer = !ME.autoLayer; obstacleAutoLayerBtn.classList.toggle('active', ME.autoLayer); };
  if (obstacleAutoOrientBtn) obstacleAutoOrientBtn.onclick = () => { ME.autoOrient = !ME.autoOrient; obstacleAutoOrientBtn.classList.toggle('active', ME.autoOrient); };
  if (obstacleAutoLayerBtn) obstacleAutoLayerBtn.classList.toggle('active', ME.autoLayer);
  if (obstacleAutoOrientBtn) obstacleAutoOrientBtn.classList.toggle('active', ME.autoOrient);
  ...
  canvas.addEventListener('mousedown',e=>{
    ...
    if(e.button===2){
      ...
      if(ho>=0){
        ...
        if (ME.selObs === ho) ME.selObs = -1; else if (ME.selObs > ho) ME.selObs--;
        meDraw();
        e.preventDefault();return;
      }
    ...
    if (ME.mode === 'obstacle') {
      ...
      // Left-click an existing obstacle to select it — the rot/scale/layer sliders
      // then edit it in place. Empty space places a new one.
      const hit = meHitTestObstacle(cx, cy);
      if (hit >= 0) { meSelectObstacle(hit); return; }
      const layer = ME.autoLayer ? meAutoLayerAt(owx, owy) : Math.round(ME.obstacleLayer || 0);
      const obs = meDefaultObstacle(ME.obstacleType, owx, owy, layer);
      // Auto-orient to the nearest spline (blockers turn broadside to block the
      // lane); when auto-orient is off the manual Rotate slider is used as-is.
      obs.rot = ME.autoOrient ? meAutoObstacleRot(ME.obstacleType, owx, owy) : Math.round(ME.obstacleRot || 0);
      ME.obstacles.push(obs);
      meSelectObstacle(ME.obstacles.length - 1);
      return;
    }
    if (ME.mode === 'wall') {
      if (ME.wpts.length >= 2) {
...
document.getElementById('me-new-btn').onclick=()=>{
  ...
  ME.obstacleRot = 0; ME.obstacleScale = 1; ME.selObs = -1;
